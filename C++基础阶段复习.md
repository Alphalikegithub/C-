...
<img src="D:\QQ\Users\DOC\Desktop\Qt\循环引用_01.png" style="zoom: 33%;" />
...
![](D:\QQ\Users\DOC\Desktop\Qt\default_delete.png)
...
void test()
{
  ...
  if (uptr) // Check if the file was successfully opened
  {
...
void test2()
{
	...
	if (sptr) // Check if the file was successfully opened
	{
...
![image-20230128154451502](D:/C++短期班/笔记/23短期班基础/23短期班笔记.assets/image-20230128154451502.png)

![image-20230128154528933](D:/C++短期班/笔记/23短期班基础/23短期班笔记.assets/image-20230128154528933.png)

#### shared_ptr的误用

![image-20230128154251275](D:/C++短期班/笔记/23短期班基础/23短期班笔记.assets/image-20230128154251275.png)

![image-20230128154229731](D:/C++短期班/笔记/23短期班基础/23短期班笔记.assets/image-20230128154229731.png)

![image-20230128154334265](D:/C++短期班/笔记/23短期班基础/23短期班笔记.assets/image-20230128154334265.png)



## Day12-2 访问容器中的元素

下面种遍历方式都能正确访问 `std::vector<std::unique_ptr<int>>` 中的元素，但它们各有**特点和潜在问题点**，我们逐一分析：
...
- 简洁且安全。
- `value` 是 `std::unique_ptr<int>&`，不会发生值拷贝或所有权转移。
- 推荐做只读或修改指针指向的内容时使用。
...
- 每次访问都需调用 `operator[]` 或 `at()`，可能比迭代器略慢（虽然差别极小）。
...
for (auto it = vec.begin(); it != vec.end(); ++it) {
    cout << *(*it) << endl; // *it 是 unique_ptr，*(*it) 是 int
}
```
...
```cpp
std::for_each(vec.begin(), vec.end(), [](const std::unique_ptr<int>& ptr) {
    cout << *ptr << endl;
});
```
...
std::ranges::for_each(vec, [](const std::unique_ptr<int>& ptr) {
    cout << *ptr << endl;
});
```
...
如果你只是**访问** `unique_ptr` 中的值，用 `const std::unique_ptr<int>&` 是最好的方式，因为：
...
- `*it` 是 `unique_ptr<int>&`，所以 `*(*it)` 就是访问 `unique_ptr` 所管理的对象，也就是 `int`。
- 所以最终你得到的是 `int` 的值。

<img src="D:\QQ\Users\DOC\Desktop\Qt\迭代器（指针）.png" style="zoom:50%;" />
...
- 所以 `*vec[i]` 是 `int`。
- 迭代器 `it` → `*it` 是 `unique_ptr<int>&`，再 `*(*it)` 就是 `int`。
...
<img src="D:\QQ\Users\DOC\Desktop\Qt\迭代器.png" style="zoom: 50%;" />
